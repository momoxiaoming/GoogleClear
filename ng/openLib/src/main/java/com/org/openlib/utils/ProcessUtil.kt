...
object ProcessUtil {
     ...
     * 获取手机已用内存
     */
    fun getUsedMemory(): Float {
        val memory = getMemory()
        val used :Float = (memory.totalMem - memory.availMem).toFloat()
        return used
    }
     ...
     * 获取手机已使用内存
     */
    fun getUsedMemoryPercent(): Long {
        val memory = getMemory()
        val used :Float = (memory.totalMem - memory.availMem).toFloat()
        return ((used / memory.totalMem) * 100).toLong()
    }
