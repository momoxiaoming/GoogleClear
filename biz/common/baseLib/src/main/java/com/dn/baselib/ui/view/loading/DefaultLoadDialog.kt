...
import com.dn.baselib.util.DrawableUtils
...
class DefaultLoadDialog constructor(context: Context) : Dialog(context), ILoadDialog {
    ...
    override fun setMessage(msg: String?) {
        msg.isNullOrEmpty().IE({},{this.msg=msg!!})
    }
    ...
    override fun setICanceledOnTouchOutside(boolean: Boolean) {
    ...
    private fun initContentView(context: Context): View {
        val content = LinearLayout(context)
        val params = FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, context.dip(50f))
        params.gravity = 17
        content.layoutParams = params
        val gradientDrawable = GradientDrawable()
        gradientDrawable.setColor(-450944201)
        gradientDrawable.cornerRadius = context.dip(5.0f).toFloat()
        content.setBackgroundDrawable(gradientDrawable)
        loadImage = ImageView(context)
        val loadParams: LinearLayout.LayoutParams = LinearLayout.LayoutParams(
            context.dip(20.0f),
            context.dip(20.0f)
        )
        loadParams.gravity = 16
        ...
        loadImage.layoutParams = loadParams
        loadImage.scaleType = ImageView.ScaleType.FIT_CENTER
        DrawableUtils.byteToDrawable(context, IMG_BYTE)?.let {
            loadImage.setImageDrawable(it)
        }
        loadMsg = TextView(context)
        loadMsg.text = msg
        loadMsg.textSize = 16.0f
        loadMsg.setTextColor(-1)
        val msgTvParams = LinearLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT)
        msgTvParams.gravity = 16
        msgTvParams.setMargins(0, 0, context.dip(17.0f), 0)
        loadMsg.layoutParams = msgTvParams
        content.addView(loadImage)
        content.addView(loadMsg)

        val root = FrameLayout(context)
        val rootParams = FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT, Gravity.CENTER)
