...
import com.mckj.cleancore.http.response.VersionInfo
import com.mckj.cleancore.data.DealData
import com.mckj.cleancore.db.DbUtil
import com.mckj.cleancore.db.JunkDatabase
import com.mckj.cleancore.manager.RetrofitManage
import okhttp3.Call
...
class InspectDbInterceptor : LInterceptor {
    ...
    fun inspectDb(dbVersionInfo: VersionInfo) {
        ...
        dbVersionInfo.apply {
            ...
            val nativeDbVersion = JunkDatabase.getInstance().junkVersionDao()
                    ...
                    ?: 0
            VLog.d(TAG + "nativeDbVersion：-->$nativeDbVersion----remoteVersion:-->$remoteVersion")
            if (nativeDbVersion < remoteVersion) {
                downloadDb()
            } else {
                VLog.d(TAG + "should not update")
                proceed()
            }
        }
    }
    ...
    private fun downloadDb() {
        ...
        if (url.isNullOrEmpty() || mRetryCount > MAX_RETRY_TIME) {
            VLog.d(TAG + "downloadDb error:retry too much: $mRetryCount")
            proceed()
            return
        }
            ...
            ?.enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    try {
                        ...
                        VLog.d("$TAG   downLoadSuccess:$url")
                        mRetryCount++
                        inputStream?.close()
                        fos?.close()
                        verifyFile()
    ...
    private fun verifyFile() {
        val downLoadPath = DbUtil.getDbDownLoadPath()
        val md5 = FileUtil.getFileMD5(downLoadPath)
        val remoteMd5 = mDealData.responseFromRemote?.MD5
        if (remoteMd5.isNullOrEmpty() || md5 != remoteMd5) {
            VLog.d(TAG, "文件校验失败:服务器md5:${remoteMd5}------本地文件md5:$md5")
            retry(downLoadPath)
            return
        }
        FileUtil.copyFile(downLoadPath, DbUtil.getDbPath())
