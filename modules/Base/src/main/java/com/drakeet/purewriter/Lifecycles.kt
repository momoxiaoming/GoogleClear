...
/**
 * 避免 DefaultLifecycleObserver 不混淆的问题
...
data class ObscureDefaultLifecycleObserverWrapper(val base: ObscureDefaultLifecycleObserver) :
...
/**
 * 避免 LifecycleEventObserver 不混淆的问题
 */
fun Lifecycle.addObserver(observer: ObscureLifecycleEventObserver) {
    addObserver(ObscureLifecycleEventObserverWrapper(observer))
}

fun Lifecycle.removeObscureObserver(observer: ObscureLifecycleEventObserver) {
    removeObserver(ObscureLifecycleEventObserverWrapper(observer))
}

interface ObscureLifecycleEventObserver {
...
data class ObscureLifecycleEventObserverWrapper(val base: ObscureLifecycleEventObserver) :
    ...
    override fun onStateChanged(
        ...
        base.onStateChanged(source, event)
    }
